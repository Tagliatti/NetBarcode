...
namespace NetBarcode.Types
{
    ...
    internal class Code128 : Base, IBarcode
    {
        ...
        private void Initialize()
        {
            ...
            _codes.Rows.Add(new object[]
                {"95", Convert.ToChar(31).ToString(), Convert.ToChar(127).ToString(), "95", "10111101000"});
            _codes.Rows.Add(new object[]
            {
                "96", Convert.ToChar(202).ToString() /*FNC3*/, Convert.ToChar(202).ToString() /*FNC3*/, "96",
                "10111100010"
            });
            _codes.Rows.Add(new object[]
            {
                "97", Convert.ToChar(201).ToString() /*FNC2*/, Convert.ToChar(201).ToString() /*FNC2*/, "97",
                "11110101000"
            });
            _codes.Rows.Add(new object[] {"98", "SHIFT", "SHIFT", "98", "11110100010"});
            _codes.Rows.Add(new object[] {"99", "CODE_C", "CODE_C", "99", "10111011110"});
            _codes.Rows.Add(new object[]
                {"100", "CODE_B", Convert.ToChar(203).ToString() /*FNC4*/, "CODE_B", "10111101110"});
            _codes.Rows.Add(new object[]
                {"101", Convert.ToChar(203).ToString() /*FNC4*/, "CODE_A", "CODE_A", "11101011110"});
            _codes.Rows.Add(new object[]
            {
                "102", Convert.ToChar(200).ToString() /*FNC1*/, Convert.ToChar(200).ToString() /*FNC1*/,
                Convert.ToChar(200).ToString() /*FNC1*/, "11110101110"
            });
        ...
        private string Encode()
        {
            ...
            foreach (var s in _formattedData)
            {
                ...
                if (eRow == null || eRow.Length <= 0)
                    throw new Exception("EC128-5: Could not find encoding of a value( " + s1 + " ) in C128 type " +
                                        _code128Type);
